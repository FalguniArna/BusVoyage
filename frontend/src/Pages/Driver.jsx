            ...
            <div ref={profileRef} style={{ position:"relative" }}>
              <div
                ...
                style={{
                  ...
                  border: profileOpen ? "2px solid #f39c12" : "2px solid rgba(243,156,18,0.35)",
              ...
              {profileOpen && (
                <div style={drPd.menu}>
                  <div style={drPd.header}>
                    <div style={{ width:38, height:38, borderRadius:"50%", background:"linear-gradient(135deg,#2d3436,#f39c12)", display:"flex", alignItems:"center", justifyContent:"center", color:"#fff", fontSize:15, fontWeight:800, flexShrink:0 }}>
                      {user?.name ? user.name.charAt(0).toUpperCase() : "D"}
                    </div>
                    <div>
                      <div style={{ fontSize:13, fontWeight:700, color:"#0f172a" }}>{user?.name}</div>
                      <div style={{ fontSize:11, color:"#64748b", marginTop:2 }}>Driver · {buses.length} bus(es)</div>
                    </div>
                  </div>
                  <div style={drPd.divider} />
                  <button style={drPd.item} onClick={() => { navigate("/"); setProfileOpen(false); }}>🏠 Home</button>
                  <button style={drPd.item} onClick={handleLogout}>↪ {t.logout}</button>
                </div>
              )}
            </div>
          </div>
...
const drPd = {
  menu: {
    ...
    borderRadius:12, padding:"6px", minWidth:200, zIndex:1000,
