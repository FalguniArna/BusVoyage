...
export default function AdminDashboard() {
  ...
  // URL is source of truth for active page — makes back button work
  const active = page || "overview";
  const setActive = (key) => { navigate(`/admin/${key}`); setSideOpen(false); };
          ...
          <div className="ad-topbar-right">
            ...
            <div ref={profileRef} style={{ position:"relative" }}>
              <div
                ...
                className="ad-avatar"
                style={{ cursor:"pointer", border: profileOpen ? "2px solid #f39c12" : "2px solid rgba(227,30,36,0.4)", boxSizing:"border-box" }}
                title="Profile menu"
              >
                AD
              </div>
              {profileOpen && (
                <div style={adPd.menu}>
                  <div style={adPd.header}>
                    <div className="ad-avatar" style={{ width:38, height:38, flexShrink:0 }}>AD</div>
                    <div>
                      <div style={{ fontSize:13, fontWeight:700, color:T.textPri }}>{user.name}</div>
                      <div style={{ fontSize:11, color:T.textHint, marginTop:2 }}>Admin · Transport Dept.</div>
                    </div>
                  </div>
                  <div style={adPd.divider} />
                  <button style={adPd.item} onClick={() => { navigate("/"); setProfileOpen(false); }}>🏠 Home</button>
                  <button style={adPd.item} onClick={() => { setActive("buses"); setProfileOpen(false); }}>🚌 Buses & Routes</button>
                  <button style={adPd.item} onClick={() => { setActive("people"); setProfileOpen(false); }}>👤 Drivers</button>
                  <div style={adPd.divider} />
                  <button style={{ ...adPd.item, color:"#E31E24" }} onClick={handleLogout}>↪ Logout</button>
                </div>
              )}
            </div>
          </div>
...
const adPd = {
  menu: {
    ...
    background:T.pageBg, border:`1px solid rgba(243,156,18,0.3)`,
    borderRadius:12, padding:"6px", minWidth:220, zIndex:1000,
    boxShadow:"0 16px 40px rgba(0,0,0,0.6)",
  },
  header: { display:"flex", alignItems:"center", gap:10, padding:"10px 10px 12px" },
  divider: { height:1, background:T.divider, margin:"4px 0" },
  item: {
    display:"block", width:"100%", textAlign:"left",
    padding:"9px 12px", fontSize:13, color:T.textMut,
