...
//   const [stats, setStats] = useState({ buses:0, active:0, complaints:0, announcements:0, students:0, lostItems:0 });
...
//   const STATS_CARDS = [
//     { label:"Total Buses",         value:stats.buses,         color:"#f39c12", light:"rgba(243,156,18,0.12)",  border:"rgba(243,156,18,0.3)",  page:"buses"     },
//     { label:"Active Now",          value:stats.active,        color:"#22c55e", light:"rgba(34,197,94,0.12)",   border:"rgba(34,197,94,0.3)",   page:"buses"     },
//     { label:"Pending Complaints",  value:stats.complaints,    color:"#E31E24", light:"rgba(227,30,36,0.12)",   border:"rgba(227,30,36,0.3)",   page:"complaints"},
//     { label:"Announcements",       value:stats.announcements, color:"#60a5fa", light:"rgba(96,165,250,0.12)",  border:"rgba(96,165,250,0.3)",  page:"announce"  },
//     { label:"Registered Students", value:stats.students,      color:"#a78bfa", light:"rgba(167,139,250,0.12)", border:"rgba(167,139,250,0.3)", page:"students"  },
//     { label:"Active Lost Items",   value:stats.lostItems,     color:"#fb923c", light:"rgba(251,146,60,0.12)",  border:"rgba(251,146,60,0.3)",  page:"lostfound" },
//   ];
...
//       <div className="ad-stats-grid">
//         {STATS_CARDS.map(st => (
//           <div key={st.label} style={{ ...crd.statCard, borderTop:`3px solid ${st.color}`, cursor:"pointer" }}
//             onClick={() => setActive(st.page)}>
...
//             );
//           })}
//           {buses.length === 0 && <div style={{ textAlign:"center", padding:24, color:T.textMut, fontSize:13 }}>No buses found. Add buses first.</div>}
//         </div>
//       </div>
//     </div>
//   );
...
//   const [sForm, setSForm]       = useState({ name:"", studentId:"", phone:"" });
//   const [saving, setSaving]     = useState(false);
//   const [search, setSearch]     = useState("");

//   const fetchDrivers  = () => axios.get(`${API}/api/admin/drivers`).then(r => setDrivers(r.data));
//   const fetchStudents = () => axios.get(`${API}/api/admin/students`).then(r => setStudents(r.data));

//   useEffect(() => { fetchDrivers(); fetchStudents(); }, []);
...
//     if (!sForm.name||!sForm.studentId||!sForm.phone) return alert("All fields required.");
//     setSaving(true);
//     try {
//       await axios.post(`${API}/api/admin/students`, sForm);
//       setSForm({ name:"", studentId:"", phone:"" });
//       setShowForm(false);
//       fetchStudents();
//     } catch(err) { alert(err.response?.data?.message || "Failed."); }
//     finally { setSaving(false); }
...
//   const handleDeleteDriver  = async (id) => { if(!window.confirm("Delete driver?"))return; await axios.delete(`${API}/api/admin/drivers/${id}`); fetchDrivers(); };
//   const handleDeleteStudent = async (id) => { if(!window.confirm("Delete student record?"))return; await axios.delete(`${API}/api/admin/students/${id}`); fetchStudents(); };

//   const filteredStudents = students.filter(s =>
//     s.name.toLowerCase().includes(search.toLowerCase()) || s.studentId.includes(search)
//   );
...
//         {[["drivers","👤 Drivers"],["students","🎓 Students"]].map(([key,lbl]) => (
//           <button key={key} onClick={() => { setTab(key); setShowForm(false); }}
//             style={{ padding:"9px 20px", borderRadius:8, border:"none", cursor:"pointer", fontSize:13, fontWeight:600, background:tab===key?"#f39c12":"rgba(255,255,255,0.06)", color:tab===key?"#fff":T.textMut }}>
//             {lbl}
//           </button>
//         ))}
//         <button onClick={()=>setShowForm(v=>!v)} style={{ ...pBtn(tab==="drivers"?"#60a5fa":"#a78bfa"), marginLeft:"auto" }}>
//           <Icon d={ICONS.plus} size={15} color="#fff" />
//           {showForm ? "Cancel" : `Add ${tab==="drivers"?"Driver":"Student"}`}
//         </button>
//       </div>

//       {/* Driver form */}
//       {tab === "drivers" && showForm && (
//         <div style={{ ...crd.card, marginBottom:20 }}>
//           <CardHead title="Add New Driver" accent="#60a5fa" />
//           <div className="ad-form-grid">
//             {[["Full Name","name","e.g. Sajib"],["Username","username","e.g. driver_sajib"],["Password","password","Set a password"]].map(([lbl,key,ph]) => (
//               <div key={key}>
//                 <div style={{ fontSize:12, fontWeight:600, color:T.textMut, marginBottom:6 }}>{lbl}</div>
//                 <input type={key==="password"?"password":"text"} placeholder={ph}
//                   value={dForm[key]} onChange={e=>setDForm(f=>({...f,[key]:e.target.value}))} style={darkInput} />
//               </div>
//             ))}
//           </div>
...
//       {/* Student form */}
//       {tab === "students" && showForm && (
//         <div style={{ ...crd.card, marginBottom:20 }}>
//           <CardHead title="Add New Student Record" sub="Student will be able to register with these details" accent="#a78bfa" />
//           <div className="ad-form-grid">
//             {[["Full Name","name","e.g. Falguni Datta Arna"],["Student ID","studentId","e.g. 231-115-207"],["Phone","phone","Phone number"]].map(([lbl,key,ph]) => (
//               <div key={key}>
//                 <div style={{ fontSize:12, fontWeight:600, color:T.textMut, marginBottom:6 }}>{lbl}</div>
//                 <input placeholder={ph} value={sForm[key]}
//                   onChange={e=>setSForm(f=>({...f,[key]:e.target.value}))} style={darkInput} />
//               </div>
//             ))}
//           </div>
//           <button onClick={handleAddStudent} style={{ ...pBtn("#22c55e"), marginTop:14 }} disabled={saving}>
...
//                 <td style={tdS}><Badge label="Driver" color="#60a5fa" bg="rgba(96,165,250,0.12)" border="rgba(96,165,250,0.3)" /></td>
//                 <td style={tdS}><button onClick={()=>handleDeleteDriver(d._id)} style={{ fontSize:11, fontWeight:700, color:"#E31E24", background:"rgba(227,30,36,0.12)", border:"none", borderRadius:6, padding:"5px 12px", cursor:"pointer" }}>Delete</button></td>
//               </tr>
//             ))}
//           />
//           {drivers.length===0 && <div style={{ textAlign:"center", padding:32, color:T.textMut, fontSize:13 }}>No drivers yet.</div>}
//         </div>
//       )}
...
//           <div style={{ ...crd.card, padding:0, overflow:"hidden" }}>
//             <DTable
//               heads={["Name","Student ID","Phone","Status","Action"]}
...
//                   <td style={tdS}>
//                     <Badge
//                       label={s.isRegistered?"Registered":"Pending"}
//                       color={s.isRegistered?"#22c55e":"#94a3b8"}
//                       bg={s.isRegistered?"rgba(34,197,94,0.12)":"rgba(148,163,184,0.12)"}
//                       border={s.isRegistered?"rgba(34,197,94,0.3)":"rgba(148,163,184,0.3)"}
//                     />
//                   </td>
//                   <td style={tdS}><button onClick={()=>handleDeleteStudent(s._id)} style={{ fontSize:11, fontWeight:700, color:"#E31E24", background:"rgba(227,30,36,0.12)", border:"none", borderRadius:6, padding:"5px 12px", cursor:"pointer" }}>Delete</button></td>
//                 </tr>
//               ))}
//             />
//             {filteredStudents.length===0 && <div style={{ textAlign:"center", padding:32, color:T.textMut, fontSize:13 }}>No students found.</div>}
//           </div>
//         </>
//       )}
//     </div>
...
//   { key:"people",     label:"Drivers & Students",icon:ICONS.driver    },
//   { key:"announce",   label:"Announcements",    icon:ICONS.announce   },
//   { key:"complaints", label:"Complaints",       icon:ICONS.complaint  },
//   { key:"lostfound",  label:"Lost & Found",     icon:ICONS.lost       },
// ];
