...
// const activeMenu = page || "dashboard";
...
//       {/* ════ SIDEBAR ════ */}
//       {/* FIX: use className to control open/close instead of fighting inline style vs CSS */}
//       <aside className={`sd-sidebar ${sbOpen ? "sd-sidebar-open" : ""}`}>
...
//             {/* FIX: always render button, show/hide via CSS class */}
...
//     flexShrink: 0,
//   },
//   userId: { color: "#2d5091", fontSize: 10, flexShrink: 0 },

//   nav:          { flex: 1, overflowY: "auto", padding: "10px 10px", minWidth: 0 },
//   sectionLabel: {
...
//   },
//   footer:  { padding: "8px 10px 16px", flexShrink: 0 },
...
//   /* status text visible on desktop */
//   .status-txt { display: inline !important; }

//   /* ── TABLET & MOBILE (< 1280px): sidebar becomes drawer ── */
...
//       z-index: 200;
//     }
//     /* FIX: open state controlled purely by this class */
...
//     .shortcut-grid { grid-template-columns: 1fr !important; gap: 8px !important; }
...
export default function StudentDashboard() {
  ...
  // Push a history entry so the browser back button returns to previous section
  const selectMenu = (id) => {
    window.history.pushState({ sdPage: id }, "", window.location.pathname);
    setActive(id);
    setSbOpen(false);
    setSideOpen(false);
  };
  ...
  const renderPage = () => {
    switch (active) {
      ...
      case "complaints":    return <Complaints />;
      case "lostfound":     return <LostFound />;
      case "profile":       return <ProfilePage user={user} onNavigate={selectMenu} onLogout={handleLogout} />;
      default:
        return (
          <DashboardHome
            user={user}
        ...
        <header style={tb.bar}>
          ...
          <div style={tb.right}>
            ...
            <div ref={profileRef} style={{ position: "relative" }}>
              <button
                style={{ ...tb.ava, cursor: "pointer", border: profileOpen ? "2px solid #3b82f6" : "2px solid rgba(59,130,246,0.35)" }}
              ...
              {profileOpen && (
                <div style={pd.menu}>
                  <div style={pd.header}>
                    <div style={pd.menuAvatar}>{initials}</div>
                    <div>
                      <div style={pd.menuName}>{user.name}</div>
                      <div style={pd.menuRole}>Student · #{user.studentId}</div>
                    </div>
                  </div>
                  <div style={pd.divider} />
                  <button style={pd.item} onClick={() => { navigate("/"); setProfileOpen(false); }}>🏠 Home</button>
                  <button style={pd.item} onClick={() => { selectMenu("profile"); setProfileOpen(false); }}>👤 My Profile</button>
                  <button style={pd.item} onClick={() => { selectMenu("routes"); setProfileOpen(false); }}>🗺️ My Routes</button>
                  <button style={pd.item} onClick={() => { selectMenu("complaints"); setProfileOpen(false); }}>💬 My Complaints</button>
                  <div style={pd.divider} />
                  <button style={{ ...pd.item, color: "#ef4444" }} onClick={handleLogout}>⎋ Sign Out</button>
                </div>
              )}
            </div>
          </div>
...
function ProfilePage({ user, onNavigate, onLogout }) {
  const initials = user.name
    ? user.name.split(" ").map(w => w[0]).join("").slice(0, 2).toUpperCase()
    : "ST";
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20, maxWidth: 680 }}>
      <div style={{ background: "#0a1628", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 16, padding: 28, display: "flex", alignItems: "center", gap: 20, flexWrap: "wrap" }}>
        <div style={{ width: 72, height: 72, borderRadius: "50%", background: "linear-gradient(135deg,#1d4ed8,#3b82f6)", border: "3px solid rgba(96,165,250,0.4)", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontSize: 26, fontWeight: 800, flexShrink: 0 }}>
          {initials}
        </div>
        <div style={{ flex: 1, minWidth: 160 }}>
          <div style={{ fontSize: 20, fontWeight: 800, color: "#f1f5f9" }}>{user.name}</div>
          <div style={{ fontSize: 13, color: "#4a6fa5", marginTop: 4 }}>Student · #{user.studentId}</div>
          <div style={{ display: "flex", gap: 8, marginTop: 10, flexWrap: "wrap" }}>
            <button onClick={() => onNavigate("dashboard")} style={{ background: "#3b82f6", color: "#fff", border: "none", borderRadius: 8, padding: "7px 16px", fontSize: 12, fontWeight: 700, cursor: "pointer" }}>
              🏠 Go to Dashboard
            </button>
            <button onClick={onLogout} style={{ background: "transparent", color: "#ef4444", border: "1px solid rgba(239,68,68,0.3)", borderRadius: 8, padding: "7px 16px", fontSize: 12, fontWeight: 700, cursor: "pointer" }}>
              ⎋ Sign Out
            </button>
          </div>
        </div>
      ...
      <div style={{ background: "#0a1628", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 16, padding: "18px 22px" }}>
        <div style={{ fontSize: 13, fontWeight: 700, color: "#e2e8f0", marginBottom: 16 }}>Profile Information</div>
        {rows.map(([label, value]) => (
          <div key={label} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 0", borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
            <span style={{ fontSize: 12, color: "#4a6fa5" }}>{label}</span>
...
const pd = {
  ...
  menuName: { color: "#f1f5f9", fontSize: 13, fontWeight: 700 },
  menuRole: { color: "#4a6fa5", fontSize: 11, marginTop: 2 },
  divider: { height: 1, background: "rgba(255,255,255,0.07)", margin: "4px 0" },
  item: {
    display: "block", width: "100%", textAlign: "left",
    padding: "9px 12px", fontSize: 13, color: "#94a3b8",
...
const CSS = `
  *, *::before, *::after { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  /* ── MOBILE ONLY (< 768px): sidebar becomes drawer ── */
  @media (max-width: 768px) {
    .sd-sidebar {
      position: fixed !important;
      top: 0; left: 0; bottom: 0;
